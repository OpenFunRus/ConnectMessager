...
import { getRoleLimits } from '../../helpers/role-policy';
...
const editMessageRoute = rateLimitedProcedure(protectedProcedure, {
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    const [settings, primaryRole] = await Promise.all([
      getSettings(),
      getPrimaryUserRole(ctx.userId)
    ]);
    const roleLimits = getRoleLimits(primaryRole);
    const textLengthLimit = roleLimits.charsPerMessage.enabled
      ? Math.min(
