import { ChannelType, ServerEvents } from '@connectmessager/shared';
...
const openDirectMessageRoute = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    // Allow self-DM for personal notes chat ("Заметки").
    ...
    const canOpen = await canUsersSeeEachOther(ctx.userId, targetUser.id);
    invariant(canOpen, {
      code: 'FORBIDDEN',
      message: 'This user is not visible for your current filter.'
    });
