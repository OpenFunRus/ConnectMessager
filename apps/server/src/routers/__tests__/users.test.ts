...
describe('users router', () => {
  ...
  test('should wipe all linked data when deleting user with wipe', async () => {
    ...
    await fs.writeFile(
      path.join(PUBLIC_PATH, `wipe-message-file-${now}.png`),
      'wipe-message-file'
    );
    await fs.writeFile(
      path.join(PUBLIC_PATH, `wipe-emoji-file-${now}.png`),
      'wipe-emoji-file'
    );
    ...
    expect(wipedMessageFile).toBeUndefined();
    expect(await fs.exists(path.join(PUBLIC_PATH, `wipe-message-file-${now}.png`))).toBe(
      false
    );
    ...
    expect(wipedEmojiFile).toBeUndefined();
    expect(await fs.exists(path.join(PUBLIC_PATH, `wipe-emoji-file-${now}.png`))).toBe(
      false
    );
  });
