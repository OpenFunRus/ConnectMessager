import { ChannelPermission, Permission } from '@connectmessager/shared';
...
describe('messages router', () => {
  ...
  test('should allow toggling reactions without explicit permission', async () => {
    const { caller: caller1 } = await initTest(1);
    const { caller: caller2 } = await initTest(2);
    ...
    const updatedMessages = await caller1.messages.get({
      channelId: 1,
      cursor: null,
      limit: 50
    });

    expect(updatedMessages.messages[0]?.reactions).toEqual(
