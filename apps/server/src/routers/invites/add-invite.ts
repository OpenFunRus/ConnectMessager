import { ActivityLogType, getRandomString, Permission } from '@connectmessager/shared';
...
const addInviteRoute = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    const needsGroupInvitePermission = !!input.roleId;
    ...
    const canProceed = needsGroupInvitePermission
      ? canManageAllInvites || canGenerateGroupInvite
      : canManageAllInvites || canGenerateUserInvite;

    if (!canProceed) {
      await ctx.needsPermission(
        needsGroupInvitePermission
