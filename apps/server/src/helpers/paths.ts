...
const getDataPath = (): string => {
  const INJECTED_DATA_PATH = process.env.CONNECTMESSAGER_DATA_PATH;
  ...
  return path.join(getAppDataPath(), 'connectmessenger');
};
...
const getMediasoupBinaryPath = (): string | undefined => {
  ...
  return path.join(
    getDataPath(),
    'mediasoup',
    CONNECTMESSAGER_MEDIASOUP_BIN_NAME || 'mediasoup-worker'
  );
};
