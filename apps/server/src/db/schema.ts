...
const DEFAULT_ROLE_RANK = 10;
const DEFAULT_ROLE_SCOPE: TRoleScope = 'global';
const DEFAULT_ROLE_LIMITS: TRoleLimits = {
...
const DEFAULT_ROLE_ABILITIES: TRoleAbilities = {
  call: false,
  videoCall: false,
  remoteDesktop: false
};
...
const roles = sqliteTable(
  'roles',
  {
    ...
    rank: integer('rank').notNull().default(DEFAULT_ROLE_RANK),
    scope: text('scope').$type<TRoleScope>().notNull().default(DEFAULT_ROLE_SCOPE),
    ...
    abilities: text('abilities', { mode: 'json' })
      .$type<TRoleAbilities>()
      .notNull()
      .default(DEFAULT_ROLE_ABILITIES),
...
const channels = sqliteTable(
  'channels',
  {
    ...
    name: text('name').notNull(),
    topic: text('topic'),
    isGroupChannel: integer('is_group_channel', { mode: 'boolean' })
      .notNull()
      .default(false),
    groupDescription: text('group_description'),
    groupFilter: text('group_filter').notNull().default('Все'),
    groupAvatarId: integer('group_avatar_id').references(() => files.id, {
      onDelete: 'set null'
    }),
...
const userLoginCodes = sqliteTable(
  'user_login_codes',
  {
  ...
  (t) => [
    ...
    index('user_login_codes_expires_idx').on(t.expiresAt),
    index('user_login_codes_created_by_idx').on(t.createdByUserId)
  ]
);

const pendingUserActivations = sqliteTable(
  ...
  (t) => [
    primaryKey({ columns: [t.userId] }),
    index('pending_user_activations_expires_idx').on(t.expiresAt)
  ]
);
