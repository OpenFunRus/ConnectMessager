...
import { categories, channels, files, messages, users } from './schema';
...
const publishChannel = async (
  ...
  previousAffectedUserIds?: number[]
) => {
  if (!channelId) return;
  ...
  const hydratedChannel = {
    ...channel.channel,
    groupAvatar: channel.groupAvatar ?? null
  };
  ...
  const affectedUserIds = await getAffectedUserIdsForChannel(hydratedChannel.id, {
    permission: ChannelPermission.VIEW_CHANNEL
  });

  if (type === 'update' && ensureUsersAccess && previousAffectedUserIds) {
    const previousSet = new Set(previousAffectedUserIds);
    const nextSet = new Set(affectedUserIds);

    const retainedUserIds = affectedUserIds.filter((id) => previousSet.has(id));
    const gainedAccessUserIds = affectedUserIds.filter((id) => !previousSet.has(id));
    const lostAccessUserIds = previousAffectedUserIds.filter((id) => !nextSet.has(id));

    if (retainedUserIds.length > 0) {
      pubsub.publishFor(retainedUserIds, ServerEvents.CHANNEL_UPDATE, hydratedChannel);
    }

    if (gainedAccessUserIds.length > 0) {
      pubsub.publishFor(gainedAccessUserIds, ServerEvents.CHANNEL_CREATE, hydratedChannel);
    }

    if (lostAccessUserIds.length > 0) {
      pubsub.publishFor(lostAccessUserIds, ServerEvents.CHANNEL_DELETE, hydratedChannel.id);
    }
  } else {
    pubsub.publishFor(affectedUserIds, targetEvent, hydratedChannel);
  }
  ...
  if (ensureUsersAccess) {
    ...
    if (type === 'update') {
      if (previousAffectedUserIds) {
        return;
      }

      if (hydratedChannel.private) {
        ...
        pubsub.publishFor(allUserIds, ServerEvents.CHANNEL_CREATE, hydratedChannel);
      }
    }
  }
};
