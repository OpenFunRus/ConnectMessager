import type { TDirectMessageConversation } from '@connectmessager/shared';
...
const getDirectMessageConversations = async (
  ...
  const conversations = (
    await Promise.all(
      rows.map(async (row) => {
        ...
        const isVisible = await canUsersSeeEachOther(userId, otherUserId);
  ...
  ).filter((item): item is TDirectMessageConversation => !!item);
...
const assertDmChannel = async (
  ...
  if (participantIds.length === 2) {
    ...
    if (otherUserId) {
      const canAccess = await canUsersSeeEachOther(userId, otherUserId);
      if (!canAccess) {
        throw new TRPCError({
          code: 'FORBIDDEN',
          message: 'This direct message is no longer available for your filter.'
        });
      }
    }
  }
