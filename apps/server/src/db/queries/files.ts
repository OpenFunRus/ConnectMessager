...
const getFileIdsByChannelId = async (channelId: number): Promise<number[]> => {
  const rows = await db
...
const getOrphanedFileIds = async (): Promise<number[]> => {
  const orphanedFileIds = await db.all<{ id: number }>(sql`
    ...
    AND NOT EXISTS (
      SELECT 1 FROM channels c WHERE c.group_avatar_id = f.id
    )
  `);
  ...
  const result = await db.get(sql`
    SELECT 
      CASE 
        ...
        AND NOT EXISTS (SELECT 1 FROM channels c WHERE c.group_avatar_id = ${fileId})
        THEN 1
        ELSE 0
      END as isOrphaned
  `);
