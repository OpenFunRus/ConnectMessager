...
import { getPrimaryUserRole, getUserRoleIds } from './roles';
...
const getChannelsForUser = async (userId: number): Promise<TChannel[]> => {
  const roleIds = await getUserRoleIds(userId);
  const selectAllChannelsWithAvatar = () =>
    db
      .select({
        channel: channels,
        groupAvatar: files
      ...
      .from(channels)
      .leftJoin(files, eq(channels.groupAvatarId, files.id));

  if (roleIds.includes(OWNER_ROLE_ID)) {
    const ownerRows = await selectAllChannelsWithAvatar().all();
  ...
  const ownRole = await getPrimaryUserRole(userId);
  const ownFilter = normalizeVisibilityFilter(ownRole);
  const [channelRows, { userPermissionMap, rolePermissionMap }, dmChannelIds] =
    await Promise.all([
      selectAllChannelsWithAvatar().all(),
  ...
  const dmOtherUserIdByChannelId = new Map<number, number>();
  await Promise.all(
    dmChannelIds.map(async (channelId) => {
      const participants = await getDirectMessageChannelParticipantIds(channelId);
      const otherUserId = participants.find((id) => id !== userId);
      if (otherUserId) {
        dmOtherUserIdByChannelId.set(channelId, otherUserId);
      }
    })
  );
  ...
  const accessibilityChecks = await Promise.all(
    allChannels.map(async (channel) => {
      if (channel.isGroupChannel) {
        const groupFilter = normalizeVisibilityFilter({
          scope: channel.groupFilter === 'Все' ? 'global' : 'filter',
          filter: channel.groupFilter
        });

        if (!canFiltersSeeEachOther(ownFilter, groupFilter)) {
      ...
      if (isPublicChannel || isDmChannelParticipant) {
        if (isDmChannelParticipant) {
          const otherUserId = dmOtherUserIdByChannelId.get(channel.id);
          if (otherUserId) {
            return {
              channel,
              allowed: await canUsersSeeEachOther(userId, otherUserId)
            };
          }
        }
      ...
      if (userPerm !== undefined) {
        return { channel, allowed: userPerm };
      }

      const rolePerm = rolePermissionMap.get(channel.id);

      if (rolePerm !== undefined) {
      return { channel, allowed: rolePerm };
      }
  ...
  return accessibilityChecks
    .filter((entry) => Boolean(entry.allowed))
...
const getAffectedUserIdsForChannel = async (
  ...
  if (channel.isGroupChannel && !channel.private) {
    ...
    const visibilityChecks = await Promise.all(
      allUserIds.map(async (userId) => {
        ...
        const userFilter = normalizeVisibilityFilter(role);
        return canFiltersSeeEachOther(userFilter, groupFilter) ? userId : null;
      })
    );

    return visibilityChecks.filter((userId): userId is number => userId !== null);
  }
