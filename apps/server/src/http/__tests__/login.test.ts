import { OWNER_ROLE_ID, sha256 } from '@connectmessager/shared';
...
describe('/login', () => {
  ...
  test('should login into invited user account with valid user invite', async () => {
    await tdb.update(settings).set({ allowNewUsers: false });
    ...
    const response = await login('cm-user-random12', 'USERINV1', 'USERINV1');
    ...
    expect(data).toHaveProperty('success', true);
    expect(data.identity).toBe('testowner');

    const updatedInvite = await tdb
      .select()
      .from(invites)
      .where(eq(invites.code, 'USERINV1'))
      .get();

    expect(updatedInvite?.uses).toBe(1);

    const usersAfter = await tdb.select().from(users);
