...
class FileManager {
  ...
  private getUniqueName = async (
    ...
    while (true) {
      const candidateName = directoryPrefix
        ? path.posix.join(directoryPrefix, fileName)
        : fileName;
      const existingFile = await db
        .select()
        .from(files)
        .where(eq(files.name, candidateName))
        .get();
  ...
  public async saveFile(
    ...
    const tempBunFile = Bun.file(tempFile.path);
    const mimeType = tempBunFile?.type || 'application/octet-stream';
    const categoryDir =
      options?.chatId !== undefined
        ...
        : null;
    const storageDir =
      options?.chatId !== undefined && categoryDir
        ? path.join(String(options.chatId), categoryDir)
        : '';
    const storageDirPosix = storageDir.replaceAll('\\', '/');
    const uniqueBaseName = await this.getUniqueName(
      tempFile.originalName,
      storageDirPosix || undefined
    );
    const storageRelativeName = storageDir
      ? path.posix.join(storageDirPosix, uniqueBaseName)
      : uniqueBaseName;
    const destinationPath = path.join(PUBLIC_PATH, ...storageRelativeName.split('/'));
    ...
    // Best-effort eager thumbnail generation to reduce first-view delays in chat.
    try {
      await ensureThumbnail(destinationPath, storageRelativeName, mimeType);
    } catch {
