import type {
  ...
  UnloadPluginContext
} from '@connectmessager/plugin-sdk';
import {
  ServerEvents,
  StreamKind,
  zPluginPackageJson,
...
class PluginManager {
  ...
  public getPluginInfo = async (pluginId: string): Promise<TPluginInfo> => {
    ...
    const serverEntryPath = path.join(
      pluginPath,
      packageJson.connectmessager.entry.server
    );

    const clientEntryPath = path.join(
      pluginPath,
      packageJson.connectmessager.entry.client
    );
    ...
    return {
      ...
      path: pluginPath,
      description: packageJson.connectmessager.description,
      version: packageJson.version,
      logo: packageJson.connectmessager.logo,
      author: packageJson.connectmessager.author,
      homepage: packageJson.connectmessager.homepage,
      entry: {
