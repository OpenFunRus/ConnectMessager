...
export const setAutoReconnectAt = (timestamp: number | null) =>
  store.dispatch(appSliceActions.setAutoReconnectAt(timestamp));
...
const applyServerBranding = (info: TServerInfo) => {
  ...
  setOrCreateMeta('apple-mobile-web-app-title', 'Connect Messager');
};

const shouldReloadForNewBuild = (info: TServerInfo) => {
  const buildId = info.buildId?.trim();
  if (!buildId) {
    return false;
  }

  const lastKnownBuildId = getLocalStorageItem(LocalStorageKey.LAST_SERVER_BUILD_ID);
  if (!lastKnownBuildId) {
    setLocalStorageItem(LocalStorageKey.LAST_SERVER_BUILD_ID, buildId);
    return false;
  }

  if (lastKnownBuildId === buildId) {
    return false;
  }

  setLocalStorageItem(LocalStorageKey.LAST_SERVER_BUILD_ID, buildId);
  const alreadyReloadedForBuild = getSessionStorageItem(
    SessionStorageKey.RELOADED_SERVER_BUILD_ID
  );

  if (alreadyReloadedForBuild === buildId) {
    return false;
  }

  setSessionStorageItem(SessionStorageKey.RELOADED_SERVER_BUILD_ID, buildId);
  return true;
};
...
export const fetchServerInfo = async (): Promise<TServerInfo | undefined> => {
  try {
    ...
    if (shouldReloadForNewBuild(data)) {
